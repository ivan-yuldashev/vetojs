...
| **Dependencies** | 4 | 0 |
| **Bundle size** | ~7.0 kB for the whole package (6.3 kB gzip to build and check). Code you never use ships anyway — `$elemMatch`, for instance, even if your policy never touches it. | 5.1 kB gzip for the same check with the incoming rules validated. 3.8 kB without validation. The whole package is 5.9 kB. |
